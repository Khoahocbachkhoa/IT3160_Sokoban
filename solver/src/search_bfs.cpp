...
std::vector<SuccessorBFS> generateSuccessorsBFS(
    const Board& board,
    const SearchNodeBFS& current,
    const DeadlockDetector& detector) 
{
    std::vector<SuccessorBFS> successors;
...
SolveResult solveBFS(const Board& board, const State& start_state, long long time_limit_ms) {
    ...
    std::queue<SearchNodeBFS> frontier;
    std::unordered_set<State, StateHash> visited;

    SearchNodeBFS start_node;
    ...
    while (!frontier.empty()) {
        ...
        SearchNodeBFS current = frontier.front(); frontier.pop();
        ...
        auto successors = generateSuccessorsBFS(board, current, detector);
