...
std::vector<SuccessorUCS> generateSuccessorsUCS(
    const Board& board,
    const SearchNodeUCS& current,
    const DeadlockDetector& detector) 
{
    std::vector<SuccessorUCS> successors;
...
SolveResult solveUCS(const Board& board, const State& start_state, long long time_limit_ms) {
    ...
    while (!frontier.empty()) {
        ...
        auto successors = generateSuccessorsUCS(board, current, detector);
